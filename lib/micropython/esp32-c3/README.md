...
In `riscv32-cross.cmake` the cmake cross-compiler setttings will be defined.

> [!WARNING]
> You **must** adjust the compiler settings in `riscv32-cross.cmake`; `CMAKE_C_COMPILER`, `CMAKE_CXX_COMPILER` and `CMAKE_FIND_ROOT_PATH` must point to valid paths and working compiler executables.
...
Install the static library (`/usr/local/lib/esp32c3/libmicropython.a`) and header file (`/usr/local/include/esp32c3/micropython_embed.h`) as root user.
